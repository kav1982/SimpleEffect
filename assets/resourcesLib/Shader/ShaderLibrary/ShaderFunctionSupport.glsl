#ifndef SHADER_FUNC_SUPPORT
#define SHADER_FUNC_SUPPORT
...
{
    mat3 worldToObjectNormalMat = mat3(inverse(worldMatrix));
    vec3 normalWS = (normalOS * worldToObjectNormalMat);
...
{
    float bendSize = u_CurveBendSize * 0.01;
    vec3 posWS = positionWS;
    posWS -= u_CurvePivotPoint;

    mediump vec2 offset = max(vec2(0.0, 0.0), abs(posWS.xz) - vec2(u_CurveBendOffset));
    offset *= step(vec2(0.0, 0.0), posWS.xz) * 2.0 - 1.0;
    offset *= offset;

    posWS = vec3(0.0, -(bendSize * offset.x + bendSize * offset.y), 0.0); 
    posWS.y *= 0.1;
    positionWS += posWS;
}
...
//人物的弯曲--高精度
void LittleCharacter_Y_Curve(inout vec3 positionWS)
{
    vec3 posWS = positionWS;
    posWS -= u_CurvePivotPoint;
...
{   
    ...
    //float fade = (worldPos.y - u_SoftParticleStartHeight) / (u_SoftParticleEndHeight - u_SoftParticleStartHeight);
    // float fade = smoothstep(u_SoftParticleStartHeight, u_SoftParticleEndHeight, worldPos.y);
    // fade = smoothstep(0.0, 1.0, exp(-fade));
